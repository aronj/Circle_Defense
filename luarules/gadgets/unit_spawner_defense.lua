...
local SKIRMISH = {
  ...
  [UnitDefNames["corthud1"].id] = { distance = 330, chance = 0.8},
  [UnitDefNames["cormist"].id] = { distance = 440, chance = 0.8},
  [UnitDefNames["cormist1"].id] = { distance = 470, chance = 0.8},
  [UnitDefNames["armsam"].id] = { distance = 440, chance = 0.8},
  [UnitDefNames["armsam1"].id] = { distance = 470, chance = 0.8 },
  [UnitDefNames["armjanus1"].id] = { distance = 400, chance = 0.6 },
  [UnitDefNames["tawf114"].id] = { distance = 800, chance = 0.9 },
  [UnitDefNames["tllcoyote"].id] = { distance = 400, chance = 0.8 },
  [UnitDefNames["corgol"].id] = { distance = 600, chance = 0.8 },
}
local COWARD = {
  ...
  --Robots
  [UnitDefNames["cortotal"].id] = { distance = 1500, chance = 0.7 },
  [UnitDefNames["heavyimpact"].id] = { distance = 1200, chance = 0.8 },
  [UnitDefNames["tlllongshot"].id] = { distance = 5000, chance = 0.8 },
  [UnitDefNames["armshock"].id] = { distance = 1400, chance = 0.8 },
  [UnitDefNames["mammouth"].id] = { distance = 900, chance = 0.8 },
  [UnitDefNames["armraven"].id] = { distance = 1200, chance = 0.7 },
  [UnitDefNames["armsnipe"].id] = { distance = 800, chance = 0.6 },
  [UnitDefNames["corcrash"].id] = { distance = 600, chance = 0.9 },
  [UnitDefNames["clb"].id] = { distance = 3000, chance = 0.9 },
  [UnitDefNames["armaak"].id] = { distance = 1500, chance = 0.9 },
  [UnitDefNames["corhrk1"].id] = { distance = 4000, chance = 0.9 },
  [UnitDefNames["roland"].id] = { distance = 1800, chance = 0.9 },
  [UnitDefNames["requ2"].id] = { distance = 4500, chance = 0.9 },
  [UnitDefNames["requ3"].id] = { distance = 6000, chance = 0.9 },
  --Robots
  ...
  if queenID then -- spawn units from queen
    if queenSpawnMult > 0 then
      for i = 1,queenSpawnMult,1 do
        local squad = waves[12][mRandom(1,#waves[12])]
...
function gadget:GameFrame(n)
    ...
    if (burrowSpawnRate < (t - timeOfLastFakeSpawn) and burrowTarget < maxBurrows) then
      ...
      if firstSpawn then
        ...
        burrowTarget = math.max(math.min(math.ceil(minBurrows * 1.0) + gracePenalty, 40), 1)
      else
	...
	if (burrowCount < minBurrows) or (burrowSpawnTime <  (t - timeOfLastSpawn) and burrowCount < maxBurrows) then
		if firstSpawn then
			for i = 1,math.min(math.ceil((SetCount(humanTeams) * 1.0)) + gracePenalty, 40),1 do
				SpawnBurrow()
			end
  ...
  if (unitDefID == burrowDef) and (not gameOver) then
    ...
    for burrowID in pairs(burrows) do
	  ...
	  if (mRandom(0,1) == 1) then bonusTurret = bonusTurret5a else bonusTurret = bonusTurret5b end
	    ...
	    if (mRandom(0,1) == 1) then bonusTurret = bonusTurret7a else bonusTurret = bonusTurret5b end
	end
       end
